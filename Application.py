...
class Application:
    ...
    def run(self):
        ...
        for i in range(self.num_workers):
            self.workers.append(Worker(self.fspath, self.DEVICE, self.num_workers, i, 3, os.getenv(
                'WORKER' + str(i+1) + '_KEY'), i < self.num_evil))
        ...
        for round in range(self.num_rounds):
            ...
            for idx, worker in enumerate(self.workers):
                ...
                unsorted_scores = [score[0].numpy().item() for score in unsorted_scores]

                # unsorted_scores =[0.9408053159713745, 0.9399038553237915]
